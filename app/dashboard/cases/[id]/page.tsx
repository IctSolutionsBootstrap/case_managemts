                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Prior Offenses & Related Cases
                    </CardTitle>
                    <CardDescription>Historical criminal records and case files dynamically matched for the defendants involved in this case.</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {(() => {
                      const currentDefendants = caseData.defendants?.map(d => d.name) || []
                      const relatedCases = mockCases.filter(c => 
                        c.id !== caseData.id && 
                        c.defendants?.some(d => currentDefendants.includes(d.name))
                      )
                      
                      if (relatedCases.length === 0) {
                        return (
                           <div className="text-center py-10">
                              <CheckCircle className="size-10 text-emerald-500 mx-auto mb-2 opacity-50" />
                              <p className="text-muted-foreground">No prior related cases found for these defendants.</p>
                           </div>
                        )
                      }
                      ...
                      return (
                        <div className="space-y-4">
                          {relatedCases.map((rc) => (
                            <div key={rc.id} className="p-5 border rounded-lg hover:bg-slate-50 transition-colors shadow-sm">
                               <div className="flex items-start justify-between mb-4">
                                  <div>
                                    <h4 className="font-black text-indigo-700 text-lg">
                                      <Link href={`/dashboard/cases/${rc.id}`} className="hover:underline flex items-center gap-2">
                                        {rc.caseNumber} <ArrowRight className="size-4" />
                                      </Link>
                                    </h4>
                                    <p className="text-sm font-bold text-slate-800 mt-1">{rc.title}</p>
                                    <p className="text-xs text-slate-500 mt-0.5 capitalize font-medium">{rc.crimeType.replace('_', ' ')} · Filed {format(new Date(rc.createdAt), 'yyyy')}</p>
                                  </div>
                                  <Badge className={cn("text-white font-bold tracking-wider", statusColors[rc.status])}>
                                    {rc.status.replace(/_/g, ' ').toUpperCase()}
                                  </Badge>
                               </div>
                               <div className="bg-slate-100/80 p-4 border border-slate-200 rounded-md text-sm text-slate-800">
                                 <span className="font-black block text-[10px] text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                                   <Scale className="size-3" /> Detailed Extracted Decision
                                 </span>
                                 <p className="leading-relaxed">
                                   {rc.status === 'sentenced' || rc.status === 'serving_sentence' 
                                     ? `The subject was found guilty and formally sentenced to serve time in the regional correctional facility. The verdict stands as a matter of public record.`
                                     : rc.status === 'dismissed' || rc.status === 'closed'
                                     ? `The statutory charges in this historical matter were formally dismissed or closed due to lack of substantive evidence or a negotiated settlement.`
                                     : `This historical matter is currently marked as active in the registry. Present Status: ${rc.status.replace(/_/g, ' ').toUpperCase()}.`}
                                 </p>
                               </div>
                            </div>
                          ))}
